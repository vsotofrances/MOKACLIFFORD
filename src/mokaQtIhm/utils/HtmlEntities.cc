...
const HtmlEntity* HTML::findEntity(const QString &entity)
{
  for (int i=0;i<FirstEntities;i++)
    if (entity == first[i].name) return &first[i];
  return NULL;
}
...
// La recherche est lineaire (non dichotomique).

QChar HTML::resolveEntity(const QString &entity)
{
  HtmlEntity* he = (HtmlEntity*) findEntity(entity);
...
void HTML::decodeStr(QString* str)
{
  ...
  while((index = entityParser.indexIn(*str,index)) != -1)
  {
    ...
    if (!decodedChar.isNull())
      str->replace(entityParser.cap(1),decodedChar);
  }
}
...
QString* HTML::decode(const string* src)
{
  return decode(src->c_str());
}
...
QString* HTML::encode(const string* src)
{
  return encode(src->c_str());
}
